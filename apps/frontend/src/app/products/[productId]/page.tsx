  ...
  try {
    ...
    if (!res.ok) {
      if (res.status === 404) {
        ...
        throw new Error(`Ürün detayları yüklenirken bir hata oluştu: ${res.statusText}`);
      }
    } else {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-300">
        ...
        <p className="text-xl font-medium text-blue-600 mb-4">${product.price?.toFixed(2)}</p>
        <p className="text-base text-gray-800 leading-relaxed mb-6">{product.description}</p>

        <button
          className="bg-blue-600 hover:bg-blue-700 text-white text-base font-medium px-5 py-2.5 rounded-lg shadow-sm transition"
        >
          Sepete Ekle
        </button>
      </div>
